...
def k_combinations(l, k):
    """
    A generator which generates all k-combinations of objects in the list l

    :param l: A list of objects

    :param k: An integer less than or equal to the number of elements in the parameter l
    """
    if k == 0:
        yield []
    else:
        for i in xrange(len(l)):
            for c in k_combinations(l[i+1:], k-1):
                yield c + [l[i]]
...
def num_partitions(n):
    """Returns the total number of ways to partition a set of n elements."""
    sum = 0
...
def gen_partitions(n):
    """
    Generator: Non recursively generates all partitions on n indistinguishable objects.
    ...
    are the size of the partitions and always sum to n. The next state is generated by
    ...
    [A] If there are 2 or more partitions of size 1, then merge any 2 partitions of size 1 into
        a single partition of size 2

    [B] If there are less than 2 partitions of size 1, then create a partition of the next
        ...
        "i" objects of the sum into a new i-partition) then new partitions of size 1 are created
        from the remaining objects and the algorithms continues. In this way a new partition of
    ...
    while lastPart.count(n) < 1 :
        if (lastPart.count(1)-2) >= 0 :
            ...
            #handle the rest of the partitions
...
def print_int_part(n):
    """Prints the output of each generation of the gen_partitions generator"""
    for i in gen_partitions(n):
        i.sort()
        print i

def num_isomorphism_classes(n):
    """
